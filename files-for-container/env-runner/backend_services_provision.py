...
class BackendServicesProvision:
    ...
    def stand_up_postgres(self):
        ...
        self.env.run_shell_command("sed -i 's/peer/trust/g' /etc/postgresql/11/main/pg_hba.conf")
        self.env.run_shell_command("sed -i 's/md5/trust/g' /etc/postgresql/11/main/pg_hba.conf")
        self.env.run_shell_command("pg_ctlcluster 11 main restart")

    def initialise_postgres_with_test_data(self):
        Environment.display_status_banner("Initialising postgres with test data...")
        postgres_user = "postgres"

        self.env.run_shell_command(f'psql --user {postgres_user} --command "CREATE DATABASE digitalmarketplace;"')
        self.env.run_shell_command(f'psql --user {postgres_user} --command "CREATE DATABASE digitalmarketplace_test;"')
    ...
    def stand_up_redis(self):
        self.env.run_shell_command("/etc/init.d/redis-server start")

    def stand_up_nginx(self):
        Environment.display_status_banner("Starting nginx...")
        self.env.run_shell_command(f"cp {self.env.runner_directory}/config/nginx.conf /etc/nginx/")
        self.env.run_shell_command("/etc/init.d/nginx start")
